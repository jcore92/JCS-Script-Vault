...
twingate_title="Twingate Travel Access"
...
require_twingate_device() {
    if [ ! -r "$twingate_state_path" ] || [ ! -r "$twingate_key_path" ]; then
...
read_twingate_state() {
    twingate_tenant="$(run_as_root cat "$twingate_state_path" 2>/dev/null | jq -r '.tenant // empty')"
    twingate_service_account_id="$(run_as_root cat "$twingate_state_path" 2>/dev/null | jq -r '.serviceAccountId // empty')"
    twingate_service_key_id="$(run_as_root cat "$twingate_state_path" 2>/dev/null | jq -r '.serviceKeyId // empty')"
    twingate_device_name="$(run_as_root cat "$twingate_state_path" 2>/dev/null | jq -r '.deviceName // empty')"

    [ -n "$twingate_tenant" ] && [ -n "$twingate_service_account_id" ] && [ -n "$twingate_service_key_id" ] && [ -n "$twingate_device_name" ]
}
...
prompt_tenant() {
    ...
    while true; do
        ...
        case "$value" in
            ""|*[!A-Za-z0-9-]*)
                echo "Enter only the tenant subdomain, for example: my-company"
                ;;
            *)
...
prompt_admin_api_key() {
    echo ""
    echo "Your Twingate Admin API key is required for this management action."
    echo "Twingate shows newly generated API keys only once, so keep yours in a password manager."
    read -r -s -p "Enter Twingate Admin API key (input hidden): " twingate_admin_api_key
    printf '\n'

    if [ -z "$twingate_admin_api_key" ]; then
...
twingate_create_service_account() {
    ...
    if [ "$ok" != "true" ]; then
        twingate_show_api_error "$response"
        return 1
    fi
...
twingate_create_service_key() {
    ...
    if [ -z "$twingate_service_key_id" ] || [ -z "$twingate_service_key_token" ]; then
        echo "Twingate created no usable Service Key token."
        return 1
    fi
...
twingate_status() {
    ...
    if [ -r "$twingate_state_path" ] && read_twingate_state; then
    ...
    if [ -r "$twingate_key_path" ]; then
        echo "Service Key file: present"
    else
        echo "Service Key file: missing"
    fi
...
twingate_revoke_key() {
    ...
    query='mutation serviceAccountKeyRevoke($id: ID!) {
      serviceAccountKeyRevoke(id: $id) { ok error }
    }'
    variables="$(jq -n --arg id "$key_id" '{id: $id}')"
...
twingate_delete_key() {
    ...
    query='mutation serviceAccountKeyDelete($id: ID!) {
      serviceAccountKeyDelete(id: $id) { ok error }
    }'
    variables="$(jq -n --arg id "$key_id" '{id: $id}')"
    response="$(twingate_api_call "$query" "$variables")" || return 1
...
twingate_delete_service_account() {
    ...
    query='mutation serviceAccountDelete($id: ID!) {
      serviceAccountDelete(id: $id) { ok error }
    }'
    variables="$(jq -n --arg id "$service_account_id" '{id: $id}')"
    response="$(twingate_api_call "$query" "$variables")" || return 1
...
twingate_setup() {
    echo ""
    echo "$twingate_title — Set up this computer"
    divider
    ...
    if [ -z "$twingate_device_name" ]; then
        echo "Unable to determine this computer's hostname."
        return 1
    fi
    ...
    if ! write_twingate_files; then
        echo "The Service Key was created remotely but could not be saved locally."
        echo "Revoke the newly created key in Twingate Admin before trying again."
        return 1
    fi
...
twingate_replace_key() {
    local old_key_id key_name answer

    require_twingate_device || return 1
    read_twingate_state || {
        echo "Local Twingate state is incomplete. Remove this computer and set it up again."
        return 1
    }
    ...
    if ! write_twingate_files; then
        echo "Replacement key was created remotely but could not be installed locally."
        return 1
    fi
    ...
    if ! twingate_revoke_key "$old_key_id"; then
        echo "The replacement key is installed, but the previous key was not revoked."
        echo "Revoke it manually in Twingate Admin: $old_key_id"
        return 1
    fi
...
twingate_remove_device() {
    ...
    require_twingate_device || return 1
    read_twingate_state || {
        echo "Local Twingate state is incomplete."
        return 1
    }

    echo ""
    echo "This stops the local Client, revokes and deletes its Service Key,"
    echo "deletes its Service Account, and removes local Twingate files."
...
twingate_open_admin() {
    ...
    xdg-open "https://${tenant}.twingate.com/" >/dev/null 2>&1 &
}
