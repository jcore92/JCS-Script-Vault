...
require_device() { device_files_exist || { echo "This computer has not been set up for Twingate Travel Access yet."; return 1; }; }
...
prompt_api_key() {
    ...
    echo ""; echo "Generate an API key, save it in your password manager, then return here."
...
create_account() {
    ...
    mutation serviceAccountCreate "$q" "$v" || { echo "Twingate API error: $twingate_last_error"; return 1; }
...
create_key() {
    ...
    mutation serviceAccountKeyCreate "$q" "$v" || { echo "Twingate API error: $twingate_last_error"; return 1; }
...
write_files() {
    ...
    key_tmp="$(mktemp)"; state_tmp="$(mktemp)" || { rm -f "$key_tmp"; return 1; }
...
start_client() {
    ...
    runtime_cmd run -d --name "$twingate_container_name" --network host --device /dev/net/tun --cap-add NET_ADMIN -v "$twingate_key_path:/etc/twingate/service_key.json:ro" "$twingate_image" || { echo "The Twingate Client container could not be started."; return 1; }
...
show_status() {
    ...
    if device_files_exist && read_state; then echo "Tenant: $twingate_tenant"; echo "Device identity: $twingate_device_name"; echo "Service Account ID: $twingate_service_account_id"; else echo "Local device state: not configured"; fi
...
remote_step() { local label="$1"; shift; "$@" && { echo "$label: complete"; return 0; }; remote_missing && { echo "$label: already absent"; return 0; }; echo "$label: $twingate_last_error"; return 1; }

local_cleanup() { require_runtime || return 1; runtime_cmd rm -f "$twingate_container_name" >/dev/null 2>&1 || true; run_as_root rm -f "$twingate_key_path" "$twingate_state_path"; echo "Local Twingate container and files were removed."; }

setup_device() {
    require_runtime || return 1
    device_files_exist && { echo "This computer already has local Twingate Travel Access files."; return 1; }
    twingate_device_name="$(hostname)"; echo "Device identity: $twingate_device_name"
    prompt_tenant && prompt_api_key && choose_expiration || return 1
    echo "Creating Service Account..."; create_account || return 1
    echo "Creating Service Key..."; create_key "$twingate_device_name-key" || return 1
    write_files || return 1; unset twingate_admin_api_key twingate_service_key_token
...
replace_key() {
    ...
    choose "Replace this computer's device key?" selection "${options[@]}"; [ "$selection" = "Create replacement key" ] || return 0
    old="$twingate_service_key_id"; prompt_api_key && choose_expiration || return 1
...
remove_device() {
    ...
    require_device && read_state || { echo "Local state is incomplete. Use local-only removal."; return 1; }
    choose "Choose removal behavior:" selection "${options[@]}"
    [ "$selection" = "Remove local files only" ] && { local_cleanup; return; }
    ...
    local_cleanup; unset twingate_admin_api_key
}
